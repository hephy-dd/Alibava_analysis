...
from analysis_classes.utilities import create_dictionary, load_parser
...
def main(args):
    ...
    if args.config:
        ...
        print(
            "AliSys needs at least the --config parameter. Type AliSys --help to see all params"
        )
        sys.exit(0)
    plot = PlotData(
        os.path.join(os.getcwd(), ext, cfg.get("plot_config_file", "plot_cfg.yml"))
    )
    results = {}
    ...
    for ped, cal, run in meas_files:
        ...
        cfg.update({"calibration": cal_data, "noise_analysis": ped_data})
        ...
        if cfg.get("run_post_analysis"):
            post_analysis_results.append(post_analysis.main(ped, cal, run, results))
        ...
        if (
            ...
            and cfg.get("Output_name", "")
            and cfg.get("Save_output", False)
        ):
    ...
    if cfg.get("run_post_analysis"):
        post_analysis.final(post_analysis_results)
