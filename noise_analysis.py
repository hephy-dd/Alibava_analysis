...
class noise_analysis:
    ...
    def __init__(self, path = ""):
        ...
        :param numchan: Number of channels which should be analysed.
        :param events:  Number of events which should be analysed
    ...
    def noise_calc(self):
        # Noise calculation, normal noise (NN) and commond mode noise (CMN)
        score = np.zeros((len(self.goodevents[0]), self.numchan), dtype=np.float32) # Variable needed for noise calculations
        for event in tqdm(self.goodevents[0], desc="Events processed:"): # Loop over all good events

            # Calculate the common mode noise for every channel
            cm = np.single(self.data['/events/signal'][event][:]) - self.pedestal  # Get the signal from event and subtract pedestal
        ...
        self.noise = np.std(score,axis=0) # Calculate the actual noise for every channel by building the mean of all noise from every event
